...
for index in mineIndices:
    ...
    results[0].append(engineArray[index[0]][index[1]]) # Modified spot-checking line to add the special symbol character for gear-checking
    mines.append(results[0])
print(mines)
print('Len of mines array',len(mines))

# Adjusted math; instead of adding to get a grand total, check to see if the mineList is a gear list. If it is, calculate gear ratio and
# add.
gearRatio = 0
for mineList in mines:
    if(len(mineList) == 3):
        if(mineList[2] == '*'):
            gearRatio += (int(mineList[0])*int(mineList[1]))
